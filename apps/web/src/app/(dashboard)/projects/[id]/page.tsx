...
//               {showKanban && (
//                 <div
//                   className={`flex flex-col overflow-hidden  border border-gray-800 bg-gray-900 ${isSplit ? "mb-2.5" : "mb-0"} ${isSplit ? "rounded-xl" : "rounded-none"}
...
//                     </span>
//                   </div>
//                   <div className="flex-1 overflow-auto p-3 max-w-7xl justify-center mx-auto ">
...
export default function KanbanPage() {
  ...
  const [activeView, setActiveView] = useState<ActiveView>(() => {
    if (typeof window !== "undefined") {
      return (
        (localStorage.getItem("kanban_active_view") as ActiveView) || "kanban"
      );
    }
    return "kanban";
  });
        ...
        >
          ...
          {showKanban && (
            ...
            >
              ...
              <div className="flex-1 overflow-auto p-6 w-full max-w-6xl  mx-auto">
                <KanbanBoard
                  project={project!}
                  projectId={Number(id)}
                  tasks={tasks}
          ...
          {showActivity && (
            <div
              className={`flex flex-col overflow-hidden border border-border bg-card 
                ${isSplit ? "mb-2.5 rounded-xl w-80 flex-shrink-0" : "mb-0 rounded-none flex-1"}`}
            >
              <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
                <svg
                  width="14"
              ...
              <div
                className={`flex-1 overflow-auto p-3 w-full max-w-4xl mx-auto ${isSplit ? "mb-0" : "mb-20"}`}
              >
                <ActivityLog logs={logs} loading={logsLoading} />
              </div>
            </div>
